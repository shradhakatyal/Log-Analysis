...
dbname = 'news'
query1 = \
    '''select articles.title, count(*) as num from articles JOIN log ON
    log.path like concat('/article/%',articles.slug)
    group by articles.title order by num desc limit 3;'''
query2 = \
    '''select authors.name, count(*) as num from authors join articles
    on authors.id = articles.author
    JOIN log ON log.path LIKE concat('/article/%',articles.slug)
    GROUP BY authors.name order by num desc;'''
query3 = \
    '''select t.day, round(((e.errors::decimal)/ t.total), 3)*100 AS
